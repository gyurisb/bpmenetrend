...
namespace CityTransitApp.WPSilverlight.Tools
{
    public static partial class Tiles
    {
        ...
        public static void UpdateTile(ShellTile tile)
        {
            ...
            IRouteStopPair pair = UserBase.LinqToSQL.TileRegister.Current.Get(id);
            if (pair == null) return;
            ...
            var timeTable = TransitBase.TransitBaseComponent.Current.Logic.GetTimetable(route, stop, now);
        ...
        {
            int id = UserBase.LinqToSQL.TileRegister.Current.Get(route, stopGroup);
            UpdateTile(Get(id));
        }
    }
}
